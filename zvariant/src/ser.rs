...
use crate::framing_offset_size::{FramingOffsetSize, FramingOffsets};
        ...
        T: ?Sized + Serialize,
    {
        match self.ser.ctxt.format() {
            ...
            EncodingFormat::GVariant => {
                let key_start = self
                    .key_start
                    .expect("start index for key in a dict not marked as expected");
                let key_offset = self.ser.bytes_written - key_start;
                let v = value.serialize(&mut *self.ser);
                let offset_size =
                    FramingOffsetSize::for_encoded_container(self.ser.bytes_written - key_start);
                offset_size.write_offset(self.ser.writer, key_offset)?;
